const ADMIN_BUILD='20260920-v10';
...
let selectedCategory='all',categoryEditMode=false,officialLoading=false,uploading=false,userPagingObserver=null,proDialogState=null,proDialogMonths=1;
const uploadQueue=[];
...
function normalizeAdminSubscription(u){const source=u?.subscription||{};let plan=String(source.plan||'none');if(!['monthly','permanent'].includes(plan)&&u?.features?.lighting)plan='permanent';return{plan,startedAt:source.startedAt||null,expiresAt:source.expiresAt||null,daysRemaining:Number(source.daysRemaining||0)||null}}
function proMetaMarkup(subscription){if(!proOnly||subscription.plan==='none')return'';const proLabel=subscription.plan==='monthly'?'包月PRO':'永久PRO';return`<div class="muted pro-meta" data-pro-meta>PRO 套餐：${esc(proLabel)} · 开通时间 ${bj(subscription.startedAt)}${subscription.plan==='monthly'?` · 到期时间 ${bj(subscription.expiresAt)}`:''}</div>`}
function userCard(u,index){const detailId=`other-export-${index}`,subscription=normalizeAdminSubscription(u),monthly=subscription.plan==='monthly',permanent=subscription.plan==='permanent';return`<article class="user" data-user-id="${esc(u.figmaUserId)}" data-pro-plan="${esc(subscription.plan)}" data-pro-started-at="${esc(subscription.startedAt||'')}" data-pro-expires-at="${esc(subscription.expiresAt||'')}">${avatarMarkup(u)}<div class="user-copy"><div class="row user-name-row"><strong><span class="number-badge">#${String(num(u.userNumber)).padStart(6,'0')}</span> ${esc(u.displayName||'未命名用户')}</strong>${u.isOnline?'<span class="badge ok">在线</span>':''}${u.isAdmin?'<span class="badge admin">管理员</span>':''}</div><div class="muted user-meta">${esc(u.figmaUserId)} · 最近打开 ${bj(u.lastOpenedAt)} · v${esc(u.pluginVersion||'-')}</div>${proMetaMarkup(subscription)}<div class="stats"><span><strong>${num(u.openCount)}</strong>打开</span><span><strong>${num(u.figmaImportCount)}</strong>Figma 导入</span><span><strong>${num(u.svgaExportCount)}</strong>SVGA 导出</span><button class="stat-button other-export-toggle" data-target="${detailId}" aria-expanded="false"><strong>${num(u.otherExportCount)}</strong>其他导出 · 查看</button><span><strong>${num(u.sequenceImportCount)}</strong>序列帧</span><div class="other-export-breakdown" id="${detailId}" hidden><span><strong>${num(u.webpExportCount)}</strong>WebP</span><span><strong>${num(u.gifExportCount)}</strong>GIF</span><span><strong>${num(u.lottieExportCount)}</strong>Lottie</span><span><strong>${num(u.webmExportCount)}</strong>WebM</span></div></div></div><div class="proButtons"><button class="btn secondary proBtn monthlyProBtn" data-plan="monthly" data-id="${esc(u.figmaUserId)}" data-on="${monthly?'1':'0'}" data-label="包月PRO">包月PRO</button><button class="btn secondary proBtn permanentProBtn" data-plan="permanent" data-id="${esc(u.figmaUserId)}" data-on="${permanent?'1':'0'}" data-label="永久PRO">永久PRO</button></div></article>`}
function applyProButtonState(button,enabled){button.dataset.on=enabled?'1':'0';button.textContent=button.dataset.label||button.textContent}
function proDialogSubscription(){const card=proDialogState?.card;return{plan:String(card?.dataset.proPlan||'none'),startedAt:card?.dataset.proStartedAt||null,expiresAt:card?.dataset.proExpiresAt||null}}
function projectedMonthlyExpiry(expiresAt,months){const now=Date.now(),current=Date.parse(expiresAt||''),base=Number.isFinite(current)&&current>now?current:now;return new Date(base+months*30*86400000).toISOString()}
function renderProSubscriptionDialog(){if(!proDialogState)return;const plan=proDialogState.plan,subscription=proDialogSubscription(),active=subscription.plan===plan,title=$('#proSubscriptionTitle'),copy=$('#proSubscriptionCopy'),stepper=$('#proMonthStepper'),confirmBtn=$('#proSubscriptionConfirm');if(plan==='monthly'){title.textContent=active?'增加包月时长':'开通包月PRO';copy.textContent=`${active?`当前到期：${bj(subscription.expiresAt)} · `:''}确认后到期：${bj(projectedMonthlyExpiry(subscription.expiresAt,proDialogMonths))}`;stepper.hidden=false;confirmBtn.hidden=false;confirmBtn.textContent=active?'确认增加':'确认开通';$('#proMonthCount').textContent=String(proDialogMonths);$('#proMonthDecrement').disabled=proDialogMonths<=1}else{title.textContent=active?'已开通永久PRO':'开通永久PRO';copy.textContent=active?'该用户已拥有永久 PRO，无需重复操作。':'确认后该用户将永久拥有 PRO 权限。';stepper.hidden=true;confirmBtn.hidden=active;confirmBtn.textContent='确认开通'}}
function openProSubscriptionDialog(button){const card=button.closest('.user');if(!card)return;proDialogState={figmaUserId:button.dataset.id,plan:button.dataset.plan,card,trigger:button};proDialogMonths=1;renderProSubscriptionDialog();$('#proSubscriptionDialog').hidden=false;requestAnimationFrame(()=>$('#proSubscriptionClose').focus())}
function closeProSubscriptionDialog(){const state=proDialogState;$('#proSubscriptionDialog').hidden=true;proDialogState=null;state?.trigger?.focus()}
function applySubscriptionToCard(card,subscription){const normalized=normalizeAdminSubscription({subscription});card.dataset.proPlan=normalized.plan;card.dataset.proStartedAt=normalized.startedAt||'';card.dataset.proExpiresAt=normalized.expiresAt||'';card.querySelectorAll('.proBtn').forEach(button=>applyProButtonState(button,button.dataset.plan===normalized.plan));const current=card.querySelector('[data-pro-meta]'),markup=proMetaMarkup(normalized);if(current)current.outerHTML=markup||'';else if(markup)card.querySelector('.user-meta')?.insertAdjacentHTML('afterend',markup)}
async function submitProSubscription(){if(!proDialogState)return;const {figmaUserId,plan,card}=proDialogState,months=plan==='monthly'?proDialogMonths:1,confirmBtn=$('#proSubscriptionConfirm');try{confirmBtn.disabled=true;confirmBtn.textContent='保存中…';const result=await request('admin-user-feature',{figmaUserId,feature:'lighting',plan,months});applySubscriptionToCard(card,result.subscription);closeProSubscriptionDialog();toast(plan==='monthly'?`已增加 ${months} 个月包月 PRO`:'已开通永久 PRO')}catch(err){toast(err.message,true);renderProSubscriptionDialog()}finally{if(confirmBtn)confirmBtn.disabled=false}}
async function usersPage(){const p=$('#page');p.innerHTML=`<div class="head"><div><h1>用户数据</h1><div class="muted">北京时间统计</div></div><button class="btn secondary" id="reloadUsers">刷新</button></div><div class="grid5" id="summary"></div><div class="toolbar"><div class="tabs"><button data-mode="online">在线</button><button data-mode="daily">当日</button><button data-mode="history">历史</button><button data-mode="pro">PRO</button></div><form class="search" id="userSearch"><input name="q" value="${esc(userSearch)}" placeholder="昵称 / Figma ID / #编号"><button class="btn">搜索</button></form></div><div class="status" id="userStatus">加载中…</div><div class="list" id="userList"></div><div class="proDialogBackdrop" id="proSubscriptionDialog" hidden><section class="proDialog" role="dialog" aria-modal="true" aria-labelledby="proSubscriptionTitle"><h2 id="proSubscriptionTitle">开通包月PRO</h2><p class="muted" id="proSubscriptionCopy"></p><div class="proMonthStepper" id="proMonthStepper" aria-label="增加月份"><button class="btn secondary" id="proMonthDecrement" type="button">−</button><strong id="proMonthCount">1</strong><button class="btn secondary" id="proMonthIncrement" type="button">+</button></div><div class="proDialogActions"><button class="btn secondary" id="proSubscriptionClose" type="button">取消</button><button class="btn" id="proSubscriptionConfirm" type="button">确认开通</button></div></section></div>`;$('#reloadUsers').onclick=()=>loadUsers();document.querySelectorAll('[data-mode]').forEach(b=>{b.classList.toggle('active',b.dataset.mode===(proOnly?'pro':userMode));b.onclick=()=>{const m=b.dataset.mode;proOnly=m==='pro';userMode=proOnly?'history':m;document.querySelectorAll('[data-mode]').forEach(x=>x.classList.toggle('active',x===b));loadUsers()}});$('#userSearch').onsubmit=e=>{e.preventDefault();userSearch=e.currentTarget.q.value.trim();loadUsers()};$('#userList').addEventListener('click',e=>{const toggle=e.target.closest('.other-export-toggle');if(toggle){const d=document.getElementById(toggle.dataset.target);if(d){const open=d.hidden;d.hidden=!open;toggle.dataset.expanded=open?'true':'false';toggle.setAttribute('aria-expanded',open?'true':'false')}return}const pro=e.target.closest('.proBtn');if(pro)openProSubscriptionDialog(pro)});$('#proMonthDecrement').onclick=()=>{proDialogMonths=Math.max(1,proDialogMonths-1);renderProSubscriptionDialog()};$('#proMonthIncrement').onclick=()=>{proDialogMonths=Math.min(120,proDialogMonths+1);renderProSubscriptionDialog()};$('#proSubscriptionClose').onclick=closeProSubscriptionDialog;$('#proSubscriptionConfirm').onclick=submitProSubscription;$('#proSubscriptionDialog').onclick=e=>{if(e.target===e.currentTarget)closeProSubscriptionDialog()};$('#proSubscriptionDialog').onkeydown=e=>{if(e.key==='Escape')closeProSubscriptionDialog()};await loadUsers()}
